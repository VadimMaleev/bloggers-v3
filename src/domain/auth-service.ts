...
import {RefreshToken, UserModel} from "../repositories/db";
...
export class AuthService {
    ...
    async createRefreshToken(login: string, expirationTime: string) {
        const user = await this.checkCredentials(login)
        return await jwtUtility.createRefreshJWT(user!._id, uuidv4().toString(), expirationTime)
    }
    ...
    async saveRefreshToken(refreshToken: string, ip: string, deviceName: string | undefined): Promise<void> {
    ...
    async updateRefreshToken(oldRefreshToken: string, newRefreshToken: string, ip: string): Promise<void> {
        ...
        const expiresAtNewToken = extractExpiresDateFromRefreshToken(newRefreshToken)
        const token = await RefreshToken.findOne({userId: userIdOldToken!, issuedAt: issuedAtOldToken!})
    ...
    async deleteRefreshToken(refreshToken: string): Promise<void> {
        ...
        const userId = extractUserIdFromRefreshToken(refreshToken)
        await RefreshToken.deleteMany({userId: userId!, issuedAt: issuedAtToken!})
    }
