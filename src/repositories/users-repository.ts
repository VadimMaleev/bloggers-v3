import {BlackListRefreshTokenType, UserDBType, UserRepType, UserType} from "./types";
import {v4 as uuidv4} from 'uuid';
import add from 'date-fns/add'
import {BlackListRefreshTokenModel, UserModel} from "./db";
...
export class UsersRepository {
    ...
    async refreshConfirmationCode(email: string): Promise<string | null> {
        ...
        const userWithNewConfirmationCode = await UserModel.findOne({'accountData.email': email})
        return userWithNewConfirmationCode!.emailConfirmation.confirmationCode
    }

    async findAccountByConfirmationCode(code: string): Promise<UserDBType | null> {
        const account = await UserModel.findOne({'emailConfirmation.confirmationCode': code})
    ...
    async confirmedAccount(accountId: string): Promise<boolean> {
        const confirmAccount = await UserModel.updateOne({_id: accountId},
    ...
    async addRefreshTokenToBlackList(refreshToken: string) {
        const payloadRefreshToken = new Buffer(refreshToken.split(".")[1], 'base64').toString()
        const expirationDate = payloadRefreshToken.split(",")[2].split(":")[1].toString().replace(/[^0-9]/g, '')
    ...
    async findRefreshTokenInBlackList(refreshToken: string): Promise<boolean> {
        const tokenInBlackList = await BlackListRefreshTokenModel.findOne({token: refreshToken}).lean()
